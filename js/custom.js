...
jQuery(document).ready(function ($) {
  ...
  /* responsive video*/
  $(".rh-container")
    .find('iframe[src*="player.vimeo.com"], iframe[src*="youtube.com"]')
    .each(function () {
      var $video = $(this);
  ...
  if ($("#coupon_code_in_modal").length > 0) {
    ...
    $change_code
      .removeClass("rh-deal-compact-btn masked_coupon woo_loop_btn coupon_btn btn_offer_block wpsm-button")
      .addClass("not_masked_coupon")
      .html("<i class='rhicon rhi-scissors fa-rotate-180'></i><span class='coupon_text'>" + couponcode + "</span>");
    ...
    $(document).on("inview", ".review_visible_circle, .top_table_block, .top_chart", function (event, visible) {
      if (visible) {
        $(".radial-progress").each(function () {
          $(this)
            .find(".circle .mask.full, .circle .fill:not(.fix)")
            .animate(
